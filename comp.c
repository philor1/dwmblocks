...
const char *getstsmods(int clk, const char *fak, char *value) {
  ...
  if (strcmp(fak, "memstat") == 0) {
    ...
    case 0:
      break;
    case 1:
      system("PID=$(ps aux | grep 'st -c dbar -n psmem' | grep -v grep | awk "
             "'{print $2}') && kill $PID || st -c dbar -n psmem -e psmem &");
      break;
    case 2:
      break;
    case 3:
  ...
  if (strcmp(fak, "cpustat") == 0) {
    ...
    case 0:
      break;
    case 1:
      system("PID=$(ps aux | grep 'st -c dbar -n htop' | grep -v grep | awk "
             "'{print $2}') && kill $PID || st -c dbar -n htop -e htop &");
      break;
    case 2:
      break;
    case 3:
...
#define CPU_NBR 4
      ...
      for (j = 0; j < CPU_NBR; ++j) {
        ...
        sscanf(line, "%s %d %d %d %d", cpu_name, &new_cpu_usage[j][0],
               &new_cpu_usage[j][1], &new_cpu_usage[j][2],
               &new_cpu_usage[j][3]);
        if (line != NULL) {
          free(line);
          line = NULL;
        ...
        other_time = new_cpu_usage[j][0] - old_cpu_usage[j][0] +
                     new_cpu_usage[j][1] - old_cpu_usage[j][1] +
      ...
      if (file != NULL) {
        ...
        cputemp = 0;
      if (cputemp >= 70)
        sprintf(value + i, "[ %s %s %s %s ]  Tmp[^c%s^%d˚C^d^]", cpu_bar[0],
                cpu_bar[1], cpu_bar[2], cpu_bar[3], color9, cputemp);
      else
        sprintf(value + i, "[ %s %s %s %s ]  Tmp[%d˚C]", cpu_bar[0], cpu_bar[1],
                cpu_bar[2], cpu_bar[3], cputemp);
    }
  }
  ...
  if (strcmp(fak, "datet") == 0) {
    ...
    case 0:
      break;
    case 1:
      system("PID=$(ps aux | grep 'st -c dbar -n calen' | grep -v grep | awk "
             "'{print $2}') && kill $PID || st -c dbar -n calen -e calen &");
      break;
    case 2:
      break;
    case 3:
  ...
  if (strcmp(fak, "ltime") == 0) {
    ...
    case 1:
      system(
          "PID=$(ps aux | grep 'st -c dbar -n calcurse' | grep -v grep | awk "
          "'{print $2}') && kill $PID || st -c dbar -n calcurse -e calcurse &");
      break;
    case 2:
      break;
    case 3:
    ...
    if (file != NULL) {
      ...
      if (devs != 0) {
        for (int l = 1; l <= devs; l++) {
          sprintf(c1,
                  "lsblk -lo RM,TYPE,NAME,VENDOR | grep '1 disk' | sed '%dq;d'",
                  l);
          file = popen(c1, "r");
          if (file != NULL) {
          ...
          sprintf(c2,
                  "lsblk -lo RM,TYPE,MOUNTPOINT /dev/%s | grep '1 part' | cut "
                  "-d '/' -f4",
                  c1);
          file = popen(c2, "r");
          if (file != NULL) {
    ...
    if (file != NULL) {
      ...
      if (torrents > 2) {
        ...
        while (count < torrents) {
          sprintf(trans,
                  "transmission-remote -l | sed '%dq;d' | awk '{ print $2 }'",
                  count);
          file = popen(trans, "r");
  ...
  if (strcmp(fak, "volume") == 0) {
    ...
    case 2:
      ...
      break;
    case 3:
      system("/usr/bin/amixer set Master toggle");
      break;
    case 4:
      system("/usr/bin/amixer -q sset Master 5%+");
      break;
    case 5:
      system("/usr/bin/amixer -q sset Master 5%-");
      break;
    }
    file = popen("amixer sget Master | tail -n 1 | cut -d ' ' -f8 | cut -b 3",
                 "r");
    if (file != NULL) {
      char mute[1];
      fscanf(file, "%s", mute);
      ...
      if (mute[0] != 'f') {
        ...
        int vol;
        file = popen(
            "amixer sget Master | tail -n 1 | cut -d ' ' -f6 | tr -d -c 0-9",
            "r");
        fscanf(file, "%d", &vol);
        fclose(file);
        ...
        if (vol == 0)
          ...
          sprintf(value + i, "%s^c%s^%s^d^", server == 0 ? "(alsa)" : "(pipe)",
                  color9, "[■■■■■■■■■■]");
        else
  ...
  if (strcmp(fak, "power") == 0) {
    ...
    if (online != 0)
      sprintf(value + i, "^b%s^^c%s^[%d%%%s]^d^", color3, color0, status,
              batbar[1]);
    else {
      if (status < 15)
        sprintf(value + i, "^b%s^^c%s^[%d%%%s]^d^", color2, color0, status,
                batbar[1]);
      else {
...
/* 	POWER
-------------------------------------------------------------------------- if
(status >= 0 && status <= 10) batbar[1] = "[□□□□□□◽]"; else if (status > 10 &&
status <= 25) batbar[1] = "[▣□□□□□◽]"; else if (status > 25 && status <= 40)
                        batbar[1] = "[▣▣□□□□◽]";
                else if (status > 40 && status <= 55)
                        batbar[1] = "[▣▣▣□□□◽]";
                else if (status > 55 && status <= 70)
...
/*		VOLUME - pulseaudio...
   ...
   "[■■■■■■■■■■]"); else if (vol > 200 && vol <= 8000) sprintf(value+i, "%s",
   "(puls)[■---------]"); else if (vol > 8000 && vol <= 15000) sprintf(value+i,
   "%s", "(puls)[■■--------]"); else if (vol > 15000 && vol <= 21000)
...
/* 	BACKLIGHT
-------------------------------------------------------------------- if (light
>= 0 && light <= 10) sprintf(value+i, "%s", "{░    }"); else if (light > 10 &&
light <= 20) sprintf(value+i, "%s", "{░░   }"); else if (light > 20 && light <=
30) sprintf(value+i, "%s", "{░░░  }"); else if (light > 30 && light <= 40)
                        sprintf(value+i, "%s", "{░░░░ }");
...
/* 		CPU
----------------------------------------------------------------------- if
(cpu_percent[j] == 0) cpu_bar[j] = "▫▫▫▫▫"; else if (cpu_percent[j] > 0 &&
cpu_percent[j] <= 25) cpu_bar[j] = "▪▫▫▫▫"; else if (cpu_percent[j] > 25 &&
cpu_percent[j] <= 50) cpu_bar[j] = "▪▪▫▫▫"; else if (cpu_percent[j] > 50 &&
cpu_percent[j] <= 75) cpu_bar[j] = "▪▪▪▫▫"; else if (cpu_percent[j] > 75 &&
cpu_percent[j] <= 100) cpu_bar[j] = "▪▪▪▪▫"; else cpu_bar[j] = "▪▪▪▪▪"; } else {
                                        cpu_percent[j] = 0;
                                        cpu_bar[j] = "▫▫▫▫▫";
                                }
------------------------------------------------------------------------------------
...
/* 	MEMORY
--------------------------------------------------------------------------- if
(memusage >= 0 && memusage <= 20) membar[1] = "◽◽◽◽◽"; else if (memusage > 20 &&
memusage <= 40) membar[1] = "◾◽◽◽◽"; else if (memusage > 40 && memusage <= 60)
                        membar[1] = "◾◾◽◽◽";
